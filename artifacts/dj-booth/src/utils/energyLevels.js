...
Just the spoken words, nothing else.`;
  ...
  // A quick, short send-off: thank the finishing entertainer and note she's moving
  // to the next stage. NO call-up of the next girl (she gets her normal intro when
  // her song starts). Style: "That was the amazing Tina. Moving on to the next stage."
  if (type === 'stage_transition') {
    return `${SYSTEM_PROMPT}

EVENT: QUICK STAGE-MOVE SEND-OFF (ONE-SONG SET, MULTI-STAGE ROTATION)

${displayName} just finished her one song on this stage.${genericNote} She is NOT done for the night — she is moving to the next stage.

RULES:
- VERY SHORT: one to two sentences MAX. Quick, simple, done.
- The core message: that was ${displayName}, and she's moving on to the next stage.
- Say "the next stage" generically — NEVER a stage number or stage name.
- Do NOT call up, mention, or tease the next entertainer — her intro handles that.
- Do NOT do a money/tip push here — keep it clean and fast.
- Do NOT mention day of the week, club name, or time-specific references.
- Do NOT use "give it up", "make some noise", "let me hear you" or any cheer-soliciting language.
- Do NOT say goodbye-flavored lines ("that's all from her", "she's done for the night") — she's moving, not leaving.

EXAMPLES (inspiration only — write something COMPLETELY ORIGINAL every time):
"[energetically] That was the amazing ${displayName}! Moving on to the next stage."
"[excitedly] The one and only ${displayName}, fellas — catch her on the next stage."
"[energetically] ${displayName}, everybody! She's headed to the next stage right now."

Write something COMPLETELY ORIGINAL. One to two sentences. Short and simple.
